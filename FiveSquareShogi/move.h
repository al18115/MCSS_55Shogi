...
inline unsigned Move::usiToFrom(const std::string& usi, bool teban) {
	if (usi[1] == '*') { //2文字目が*なら打ち駒
		...
		switch (usi[0])
		{
			...
			case 'S': return from + 1;
			case 'B': return from + 2;
			case 'R': return from + 3;
			case 'G': return from + 4;
			default:assert(0); return 0;
				break;
		}
	}
	else {
		return (usi[0] - '1') * 5 + (usi[1] - 'a'); //1文字目が筋 2文字目が段
	}
}

inline unsigned Move::usiToTo(const std::string& usi) {
	return (usi[2] - '1') * 5 + (usi[3] - 'a'); //3文字目が筋 4文字目が段
}
