...
void JosekiDataBase::open(){
	...
	std::cout << "open db[" << filePath << "]" << std::endl;
	auto status = sqlite3_open_v2(filePath.c_str(), &db, SQLITE_OPEN_CREATE | SQLITE_OPEN_READWRITE, nullptr);
	if (status != SQLITE_OK)
	{
		std::cout << "error for opening db : " << sqlite3_errmsg(db) << std::endl;
	}
	else
	{
